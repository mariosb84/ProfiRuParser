...
/* Хранилище сессий: sessionId -> login *//*
    ...
    private final Map<String, String> activeSessions = new ConcurrentHashMap<>();
    */
/* Хранилище логинов: login -> sessionId (для поиска существующих сессий) *//*

    private final Map<String, String> loginToSession = new ConcurrentHashMap<>();
    */
/* 🔥 ДОБАВЛЯЕМ: Хранилище браузеров для сессий *//*

    private final Map<String, WebDriver> sessionBrowsers = new ConcurrentHashMap<>();
...
/* Создаем новую сессию *//*
    ...
    */
/* 🔥 НОВЫЙ МЕТОД: Сохранить браузер для сессии *//*

    public void attachBrowserToSession(String sessionId, WebDriver browser) {
        sessionBrowsers.put(sessionId, browser);
        System.out.println("🔗 Browser attached to session: " + sessionId);
    }

    */
/* 🔥 НОВЫЙ МЕТОД: Получить браузер сессии *//*

    public WebDriver getBrowserForSession(String sessionId) {
        WebDriver browser = sessionBrowsers.get(sessionId);
        System.out.println("🔍 Getting browser for session " + sessionId + ": " + (browser != null ? "FOUND" : "NULL"));
        return browser;
    }
    ...
    public void invalidateSession(String sessionId) {
        ...
        if (login != null) {
            */
/* 🔥 ЗАКРЫВАЕМ БРАУЗЕР ПРИ ВЫХОДЕ ИЗ СЕССИИ *//*

            WebDriver browser = sessionBrowsers.get(sessionId);
            if (browser != null) {
                try {
                    browser.quit();
                    System.out.println("🧹 Browser closed for session: " + sessionId);
                } catch (Exception e) {
                    System.out.println("⚠️ Error quitting browser: " + e.getMessage());
                }
                sessionBrowsers.remove(sessionId);
            }

            activeSessions.remove(sessionId);
            loginToSession.remove(login);
            System.out.println("🗑️ Invalidated session for user: " + login);
        }
    }
